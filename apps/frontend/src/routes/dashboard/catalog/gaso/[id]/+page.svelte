...
$effect(() => {
  if (bundle) {
    ...
    try {
        ...
        const newInst: Record<string, string> = { "3": "", "6": "", "9": "", "12": "", "18": "" };
...
function validate(): boolean {
  ...
  const catError = validateRequired(formData.primaryCategory, "La categoría");
...
function goBack() {
    ...
    // Note: We'd ideally like to switch to the 'gaso' tab but there is no URL param for tab yet.
...
async function handleSave() {
  ...
  try {
     ...
     const schedule = MONTH_OPTIONS
        .filter(m => installments[m] && parseFloat(installments[m]) > 0)
        .map(m => {
    ...
    if (!bundle) {
        ...
        await fetchApi("/api/catalog/bundles", { method: "POST", body: payload });
    ...
    // Edit Mode
        ...
        if (image) {
            ...
            await fetchApi(`/api/catalog/bundles/${bundle.id}/image`, { method: "POST", body: imgForm });
        }
    ...
    <div class="bg-white rounded-lg shadow-sm border border-ink-900/10 p-8 space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="md:col-span-2 space-y-6">
                 <FormField label="Nombre Comercial*" for="name" error={errors.name}>
                    <Input id="name" bind:value={formData.name} placeholder="Ej. Combo Cocina + Licuadora" />
                </FormField>

                <div class="grid grid-cols-2 gap-4">
                    <FormField label="Precio Contado (S/)*" for="price" error={errors.price}>
                        <Input type="number" id="price" bind:value={formData.price} />
                    </FormField>
                    
                    <FormField label="Categoría Principal*" for="category" error={errors.primaryCategory}>
                        <Input id="category" bind:value={formData.primaryCategory} placeholder="Ej. Cocinas" />
                    </FormField>
                </div>
            </div>
        </div>
        ...
        <div class="border-t border-ink-100 pt-6">
            <h4 class="text-lg font-bold text-ink-900 mb-4">Composición del Paquete</h4>
            <p class="text-sm text-ink-500 mb-4">Selecciona los productos base que componen este paquete. Se pueden agregar productos fijos.</p>
            ...
            <div class="space-y-3 bg-gray-50 p-4 rounded-md">
                {#each items as itemId, i}
                    <div class="flex gap-2 items-center">
                        <span class="text-sm font-medium w-8 text-center text-ink-400">{i + 1}.</span>
                        <div class="flex-1">
                            <Select 
                                    value={itemId}
                                    items={productOptions}
                                    placeholder="Seleccionar producto..."
                                    onchange={(e) => updateItem(i, (e.currentTarget as any).value)}
                            />
                        </div>
                        <Button variant="ghost" onclick={() => removeItem(i)} class="px-2 text-red-600 hover:bg-red-50">×</Button>
                    </div>
                {/each}
                <Button variant="outline" onclick={addItem} class="w-full text-sm mt-2 border-dashed">+ Agregar producto al paquete</Button>
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                {#each MONTH_OPTIONS as months}
                    <FormField label={`${months} meses`}>
                        <Input type="number" bind:value={installments[months]} placeholder="S/" class="text-right" />
                    </FormField>
                {/each}
            </div>
        </div>
        ...
        <div class="pt-6 border-t border-ink-900/10 flex justify-end gap-3">
             ...
             <Button onclick={handleSave} disabled={isSaving}>{isSaving ? "Guardando..." : "Guardar Paquete"}</Button>
        </div>
    </div>
</div>
